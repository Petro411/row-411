...
const Table: React.FC = () => {
  const gridRef = useRef<AgGridReact<ClientData>>(null);
  ...
  const [columnDefs] = useState<ColDef<ClientData>[]>([
    {
      headerName: "Select",
  ...
  const defaultColDef = useMemo<ColDef<ClientData>>(
    () => ({
      flex: 1,
      minWidth: 100,
      filter: true,
  ...
  const [filteredCount, setFilteredCount] = useState<number>(rowData.length);
